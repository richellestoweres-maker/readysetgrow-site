...
function tabList() {
  const kid = activeChild();
  const nm = kid && kid.name ? kid.name.trim() : '';
  /* A long name would push the other four labels out of shape, so it
     gets a first name and a cap rather than a scrollbar. */
  const short = nm ? nm.split(/\s+/)[0].slice(0, 9) : 'Child';
  return [
    { id: 'today', label: 'Today', icon: 'home' },
    { id: 'now', label: 'Right Now', icon: 'heart' },
    { id: 'child', label: short, icon: 'leaf' },
...
function render() {
  ...
  if (v && v.type === 'screen' && CHILD_SCOPED.indexOf(v.id) !== -1 && !activeChild()) {
    v = null; state.view = null; state.tab = 'today';
  }
  if (v && v.type === 'log' && !activeChild()) { v = null; state.view = null; state.tab = 'today'; }
  ...
  else if (v && v.type === 'screen' && v.id === 'profile') html = screenProfile();
  ...
  // Me is the parent's own tab and never asks which child you mean, which
  // is the whole point of it being a tab rather than a fallback.
  else if (state.tab === 'me') html = screenMe();
  // Right Now carries the emergency card and the search box whether or not
  // a child is open, because neither of those should wait for a profile.
  else if (state.tab === 'now') html = screenNow(c);
  else if (state.tab === 'logs') html = activeChild() ? screenLogs(c) : screenPickChild('Logs');
  else if (state.tab === 'child') html = activeChild() ? screenChild(c) : screenPickChild('This part of the app');
  else html = activeChild() ? screenToday(c) : screenPickChild('Today');
  ...
  document.getElementById('tabs').innerHTML = tabList().map((t) => `
    <button class="tab" role="tab" data-tab="${t.id}" aria-selected="${state.tab === t.id && !state.view}">
      ${icon(t.icon, 20, state.tab === t.id && !state.view ? 'var(--deep)' : 'var(--muted)')}
...
function initControls() {
  ...
  document.addEventListener('change', (e) => {
    if (e.target.matches('[data-childbday]')) {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (t.dataset.auth) {
      ...
      if (a === 'mode') { auth.mode = t.dataset.val; auth.error = ''; }
        ...
        store.guest = true;
        flushStore();
        state.tab = IS_DESKTOP ? 'today' : 'welcome';
      }
      ...
      document.getElementById('bdayIn').value = state.birthday;
...
function screenNow(c) {
  ...
  ${cornerLeaves()}
  <div class="sc-head">
    <p class="eyebrow">What is happening right now</p>
...
function screenWelcome(c) {
  ...
  <div class="sc" style="margin-top:26px">
    ...
    <button class="btn" data-tab="today">Start ${icon('chev', 15, '#fff')}</button>
...
function screenProfile() {
  ...
  const field = (label, key, type, placeholder, help) => `
    <div class="card flat" style="margin-bottom:8px">
      ...
      <input class="inp" type="${type}" data-parentfield="${esc(key)}" id="pf_${esc(key)}"
        value="${esc(p[key] || '')}" placeholder="${esc(placeholder)}" autocomplete="off"
  ...
  <div class="sc-head">
    ...
    <p class="sub">You, and everyone you are raising.</p>
  </div>
  <div class="sc">
    ...
    <p class="tiny" style="margin:-4px 0 10px">
      Each child keeps their own age, their own support lenses, and their own notes. Nothing crosses over.
    </p>
    ${store.children.map((k) => `
      <div class="card" style="margin-bottom:10px">
        <div style="display:flex;align-items:center;gap:9px">
          ${growthSVG((() => { const su = getAgeSummary({ name: k.name, birthday: k.birthday });
            const g = su.stageId ? getGrowthDetail(su.stageId) : null; return g ? g.order : 0; })(), 32)}
          <input class="inp" type="text" data-childname="${esc(k.id)}" id="cn_${esc(k.id)}"
            value="${esc(k.name)}" placeholder="Name" style="flex:1;min-width:0" />
        </div>
        <div style="display:flex;align-items:center;gap:9px;margin-top:9px">
          <span class="tiny" style="width:66px">Birthday</span>
          <input class="inp" type="date" id="cb_${esc(k.id)}" data-childbday="${esc(k.id)}"
            value="${esc(k.birthday || '')}" style="flex:1;min-width:0" />
        </div>
        <p class="tiny" style="margin-top:8px">${esc(childAgeLabel(k))}. ${esc(childLensLine(k))}.</p>
        <div style="display:flex;gap:7px;margin-top:10px;flex-wrap:wrap">
          <button class="chip" data-child="${esc(k.id)}">Open ${esc(k.name || 'this profile')}</button>
          <button class="chip" data-removechild="${esc(k.id)}">Remove</button>
        </div>
      </div>`).join('')}

    <div class="card">
      <p class="eyebrow">${icon('plus', 11, 'var(--sage)')} Add a child</p>
      ...
      <input class="inp" type="date" id="newChildBday" value="${esc(store.draftChildBday)}"
        style="margin-top:8px;width:100%" />
      <p class="tiny" style="margin-top:8px">
        Expecting? Leave the date as the due date and the app will follow the pregnancy instead.
      </p>
      <button class="chip" data-addchild="1" style="margin-top:10px">Add this child</button>
    </div>
    ...
    ${(() => {
      /* Which copy of the app is this device running. Put here because
         "it works on my laptop but not my phone" is almost always two
         different copies, and until this line existed there was no way
         to tell them apart from the screen. */
      const st = cloudStatusLine();
      return `
      <div class="card flat">
        <p class="eyebrow">${icon('info', 11, 'var(--sage)')} This device</p>
        <p class="tiny" style="margin-top:5px">App version ${esc(buildStamp() || 'unknown')}</p>
        <p class="tiny" style="margin-top:3px">${esc(st.title)}</p>
        ${update.available ? `
          <p class="tiny" style="margin-top:6px;color:var(--attention)">
            Version ${esc(update.stamp)} is available.
          </p>
          <button class="btn sm" style="margin-top:9px;width:100%" data-update="go">Update now</button>
        ` : `<button class="chip" style="margin-top:9px" data-update="check">Check for an update</button>`}
      </div>`;
    })()}
  </div>`;
}
...
function topBar(markOnly) {
  ...
  const act = store.activeChildId;
  const meOn = state.tab === 'me' && !state.view;
  ...
  return updateBar() + `
  <div class="topbar">
    <div class="topbar-kids">
      ${kids.map((k) => `
        <button class="kidchip${k.id === act ? ' on' : ''}" data-child="${esc(k.id)}">
          <span class="kidchip-av">${growthSVG(chipGrowthOrder(k), 19)}</span>
          <span class="kidchip-name">${esc(k.name || 'Unnamed')}${isExampleChild(k) ? ' (example)' : ''}</span>
        </button>`).join('')}
      <button class="kidchip add" data-go="screen" data-id="profile" aria-label="Add a child">
        ${icon('plus', 15, 'var(--sage)')}
      </button>
    </div>
    ${mark}
    ${IS_DESKTOP ? `
    <div class="me-slot">
      <button class="mebtn${meOn ? ' on' : ''}" data-tab="me" aria-label="You">
        ${icon('user', 15, meOn ? '#fff' : 'var(--deep)')}<span>Me</span>
      </button>
    </div>` : ''}
  </div>`;
}
...
function screenMe() {
  const hour = new Date().getHours();
  const greet = hour < 12 ? 'Good morning' : hour < 18 ? 'Good afternoon' : 'Good evening';
  const who = store.parent.name ? ', ' + store.parent.name : '';
  const kids = store.children;
  ...
  ${cornerLeaves()}
  <div class="sc-head">
    <p class="eyebrow">${esc(greet)}${esc(who)}</p>
  ...
  <div class="sc">
    ...
    <p class="tiny" style="margin:-4px 0 10px">
      This part of the app is about you, not about any of them, and it does not disappear when you open a child.
    </p>
    ...
    ${childRow('people', 'Community',
      ...
      'data-go="screen" data-id="community"')}

    <p class="sect">Your children</p>
    ${duplicateCard()}
    ${kids.length
      ? kids.map((k) => childCard(k, k.id === store.activeChildId)).join('')
      : `<div class="card flat">
           <p class="eyebrow">${icon('leaf', 11, 'var(--sage)')} No profiles yet</p>
           <p class="bodytext" style="margin-top:5px">
             Add a child and everything in the app reshapes around their age, from sleep windows to
             what to say at bedtime. Add as many as you like. Each one keeps their own notes.
           </p>
         </div>`}
    ${childRow('plus', 'Add a child', 'Name and birthday is all it takes to start',
      'data-go="screen" data-id="profile"')}
    ...
    ${childRow('star', 'Profile and settings',
      store.parent.name
...
function childPayload(k) {
  return JSON.parse(JSON.stringify({
    ...
    lenses: k.lenses || [],
    lensOptions: k.lensOptions || {},
    lensNumbers: k.lensNumbers || {},
    statuses: k.statuses || {},
    wakeTime: k.wakeTime || '06:30',
    napOverride: k.napOverride == null ? null : k.napOverride,
    routineInclude: k.routineInclude || [],
    logs: Array.isArray(k.logs) ? k.logs : [],
    createdAt: k.createdAt || null,
    updatedAt: k.updatedAt || 0,
  }));
}
/* The flag is deliberately not in the payload above. It describes where
   a record came from in this browser, and once it reaches an account it
   is simply one of her children. */
...
function mergeChildren(local, remote, deletedIds) {
  ...
  Object.keys(byId).forEach((id) => {
    ...
    if (pair.remote && !pair.local) { out.push(pair.remote); adopted.push(id); return; }
    const lt = Number(pair.local.updatedAt) || 0;
    const rt = Number(pair.remote.updatedAt) || 0;
    if (rt > lt) { out.push(pair.remote); adopted.push(id); }
    else {
      out.push(pair.local);
...
function cloudListen() {
  ...
  cloud.unsub = fs.onSnapshot(kidsRef, (snap) => {
    ...
    snap.forEach((d) => {
      ...
      if (i === -1) { store.children.push(remote); changed = true; return; }
      const lt = Number(store.children[i].updatedAt) || 0;
      const rt = Number(remote.updatedAt) || 0;
      if (rt > lt) { store.children[i] = remote; changed = true; }
    });
    if (changed) {
      if (store.activeChildId && !store.children.some((k) => k.id === store.activeChildId)) {
        store.activeChildId = null;
